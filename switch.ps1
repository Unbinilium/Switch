#Switch v0.1 written by Unbinilium
...
Write-Host "114DNS, Google, Cloudflare, Aliyun, CNNIC, Baidu, Dnspod, OpenDNS, Microsoft"
...
foreach ($DNS in $DNS_Provider.Split(" ")) { $DNS_Address = (Select-String -Path $DNS_List_TMP -Pattern "$DNS") -split "/" | Select-Object -Skip 1 -First 1; Out-File -FilePath "$DNS_Server_TMP" -InputObject "$DNS_Address," -Append -NoNewline; }
...
foreach ($Net_Adapter in $Net_Adapter_Index) { Set-DnsClientServerAddress -InterfaceIndex "$Net_Adapter" -ServerAddresses ($DNS_Server) -Validate }
